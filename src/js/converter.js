...
define([ 'jquery', 'tinymce' ], function($, tinymce) {
    ...
    return function(writer) {
        ...
        converter.getDocumentContent = function(includeRDF, separateRDF) {
            ...
            entNodes = $('[_entity][class~="start"]', body).not('[_tag]').not('[_note]');
            entIds = $.map(entNodes, function(val, index) {
            ...
            root = body.children('[_tag=' + w.root + ']');
            // Make sure the root has the right namespaces for validation
            // purposes.
            struct = w.structs[root.attr('id')];
            // Add them to the structs entry and they'll get added to the markup
            // later.
        ...
        function _nodeToStringArray(node) {
            ...
            if (entityEntry && tag) {
                ...
                openingTag = '<' + tag;
                cwrcAnnotationId = node[0].getAttribute('annotationId');
                if (cwrcAnnotationId !== null) {
                    openingTag += ' annotationId="' + cwrcAnnotationId + '"';
                }
                cwrcOffsetId = node[0].getAttribute('offsetId');
                if (cwrcOffsetId !== null) {
                    openingTag += ' offsetId="' + cwrcOffsetId + '"';
                }
                for (key in structEntry) {
                    if (structEntry.hasOwnProperty(key)) {
                        if (key.indexOf('_') !== 0) {
                            attName = key;
                            if (attName !== 'id') {
        ...
        function buildAnnotations(format) {
            ...
            w.entitiesManager.eachEntity(function(id, entity) {
                ...
                annotation = getAnnotationForEntity(id, format);
                ...
                if (entity.getCustomValues().entities && entity.getCustomValues().content) {
                    ...
                    xml = w.utilities.stringToXML(entity.getCustomValues().content);
                    rdf = $('rdf\\:RDF, RDF', xml);
