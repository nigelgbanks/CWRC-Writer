    ...
    return function(writer) {
        ...
        tagger.insertBoundaryTags = function(id, type, range, tag) {
            ...
            if (w.schemaManager.mapper.isEntityTypeNote(type)) {
                ...
                if (range.startContainer.parentNode !== range.endContainer.parentNode) {
                    nodes = w.utilities.getNodesInBetween(range.startContainer, range.endContainer);
                    startRange = range.cloneRange();
                    startRange.setEnd(range.startContainer, range.startContainer.length);
                    start = w.editor.dom.create('span', {
                        '_entity': true,
                        '_type': type,
                        'class': 'entity ' + type + ' start',
                        'name': id
        ...
        tagger.getCorrespondingEntityTag = function(tag) {
            ...
            if (tag.hasClass('start')) {
                return tagger.findEntityBoundary('end', tag[0].nextSibling);
            }
            return tagger.findEntityBoundary('start', tag[0].previousSibling);
        };
        ...
        tagger.findEntityBoundary = function(boundaryType, currentNode) {
            ...
            function doFind(boundaryType, currentNode, entIds, levels, structIds) {
                ...
                if (w.editor.dom.hasClass(currentNode, 'entity')) {
                    nodeId = currentNode.getAttribute('name');
            ...
            return doFind(boundaryType, currentNode, [], [currentNode.parentNode], {});
        };
