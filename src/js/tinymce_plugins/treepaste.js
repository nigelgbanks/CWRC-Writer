...
(function() {
    ...
    tinymce.create('tinymce.plugins.TreePastePlugin', {
        init: function(ed, url) {
            ...
            // contents forceRich overrides plain text mode set by user,
            // important for pasting with execCommand
            ...
            function grabContent(e) {
                ...
                dom.setStyles(n, {
                    position: 'absolute',
                    left: tinymce.isGecko ? -40 : 0, // Need to move it out of site on Gecko since it will otherwise display a ghost resize rect for the div
                    top: posY - 25,
                    width: 1,
                    height: 1,
                    overflow: 'hidden'
                ...
                if (tinymce.isIE) {
                    ...
                    window.setTimeout(function() {
                        ...
                        if (ed.copiedElement.element !== null) {
                            idEl = $('[id]', ed.copiedElement.element);
                            id = idEl.attr('id');
                            idString = 'id="' + id + '"';
                            ...
                            if (h.search(idString) !== -1) {
                                if (ed.copiedElement.selectionType === 0) {
                                    ...
                                    // contents selected
                                    span = $(ed.writer.tagName, ed.copiedElement.element).first()[0];
                                    if (span) {
                                        h = span.innerHTML;
                                    }
                                }
        ...
        _preProcess: function(pl, o) {
            ...
            function removeClasses(match, g1) {
                ...
                cls = grep(explode(g1.replace(/^([''])(.*)\1$/, '$2'), ' '), function(v) {
                    return (/^(?!mso)/i.test(v));
                });
        ...
        _insert: function(h, skipUndo) {
            ...
            if (tree.selectionType !== null) {
                ...
                // jscs:disable requireCamelCaseOrUpperCaseIdentifiers
                ed.execCommand('mceInsertContent', false, h, { skip_undo: skipUndo });
                // jscs:enable requireCamelCaseOrUpperCaseIdentifiers
            }
        },
        ...
        _insertPlainText: function(content) {
            ...
            if ((typeof(content) === 'string') && (content.length > 0)) {
                ...
                process([
                    ...
                    [ /<br[^>]*>|<\/tr>/gi, '\n' ],
                    // Table cells get tabs between them.
                    [ /<\/t[dh]>\s*<t[dh][^>]*>/gi, '\t' ],
                ...
                maxLinebreaks = Number(getParam(ed, 'paste_max_consecutive_linebreaks'));
                if (maxLinebreaks > -1) {
                    maxLinebreaksRegex = new RegExp('\n{' + (maxLinebreaks + 1) + ',}', 'g');
                    ...
                    process([
                        // Limit max consecutive linebreaks.
                        [ maxLinebreaksRegex, linebreakReplacement ]
                    ]);
                }
